		...
		switch (strtolower($property)) {
			case "maxredirects":
				if(is_int($value)) {
					...
					throw new PesapalException("cannot implicitly convert type in the property \"$property\" to int");
				}
				break;
			case "curl":
			case "error":
			...
			case "status":
			case "statustext":
				throw new PesapalException("property \"$property\" cannot be assigned to -- it is read only");
				break;
			default:
				throw new PesapalException("class \"".__CLASS__."\" does not contain a definition for \"$property\"");
		}
	}
  ...
  public function __get($property) {
	...
	public function __construct(){
		if(function_exists("curl_init")){
			...
			throw new PesapalException("Could not initialize cURL library");
		}
	}
	...
	public function open($method, $url, $async = false, $user = "", $password = ""){
		...
		if(!empty($method) && !empty($url)){
			...
			//if(preg_match("^(GET|POST|HEAD|TRACE|PUT|DELETE|OPTIONS)$", $method)===false){
			//	throw new Exception("Unknown HTTP method \"$method\"");
			//}
			if(!in_array($method,array("GET","POST","HEAD","TRACE","PUT","DELETE","OPTIONS")))
			{
				throw new PesapalException("Unknown HTTP method \"$method\"");	
			}
	...
	public function send($data=null){
		...
		//echo "b4 curl_exec<br/>";
		$response = curl_exec($this->curl);
		//echo "b4 curl_getinfo<br/>";exit;
